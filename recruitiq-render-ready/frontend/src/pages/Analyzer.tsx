  ...
  const freeBadge = () => {
    ...
    return (
      <div className={`mt-3 inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-semibold border ${
        left === 0 ? 'bg-red-500/10 border-red-500/25 text-red-400'
        : left === 1 ? 'bg-yellow-500/10 border-yellow-500/25 text-yellow-400'
        ...
        {!analysis ? (
          <div className="space-y-5">
            ...
            <div
              ...
              className={`relative border-2 border-dashed rounded-2xl p-10 sm:p-14 text-center transition-all ${
                  ...
                  <div className="flex justify-between relative z-10">
                    {STEPS.map((s, i) => (
                      <div key={s} className="flex flex-col items-center gap-2 flex-1">
                        <div className={`w-7 h-7 sm:w-8 sm:h-8 rounded-full border-2 flex items-center justify-center text-xs font-bold transition-all duration-500 ${
              ...
              <div className="flex items-start gap-3 p-4 sm:p-5 bg-red-500/8 border border-red-500/15 rounded-xl">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-red-300 font-semibold text-sm">
                    {upgradeRequired ? 'Free Limit Reached' : 'Analysis Failed'}
                  </p>
                  <p className="text-red-400/70 text-sm mt-0.5">{error}</p>
                  {upgradeRequired && (
                    <Link to="/pricing"
                      className="inline-flex items-center gap-1 mt-2 text-indigo-400 hover:text-indigo-300 text-sm font-semibold">
                      Upgrade to Pro →
                    </Link>
                  )}
                  {!upgradeRequired && (
                    <button onClick={handleUpload}
                      className="inline-flex items-center gap-1.5 mt-2 text-zinc-400 hover:text-white text-sm transition-colors">
            ...
            {file && !uploading && !upgradeRequired && (
              <button onClick={handleUpload}
                className="w-full py-3.5 sm:py-4 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-xl transition-all flex items-center justify-center gap-2 text-base sm:text-lg">
                <Target size={20} /> Analyze Resume
              </button>
            )}
            {upgradeRequired && (
              <Link to="/pricing"
                className="block w-full py-3.5 sm:py-4 bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-500 hover:to-violet-500 text-white font-bold rounded-xl text-center text-base sm:text-lg transition-all">
          ...
          <div className="space-y-5">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div className="flex items-center gap-2 text-emerald-400 font-medium text-sm sm:text-base">
                <CheckCircle size={17} /> Analysis Complete —{' '}
                <span className="text-zinc-400 font-normal text-sm truncate max-w-[180px] sm:max-w-xs">{analysis.file_name}</span>
              </div>
              <button
                onClick={() => { setAnalysis(null); setFile(null); setJobMatch(null); setJobDesc(''); setError(''); setJobError(''); if (fileRef.current) fileRef.current.value = ''; }}
                className="w-full sm:w-auto px-4 py-2.5 glass glass-hover text-sm text-zinc-400 hover:text-white rounded-xl transition-colors text-center">
                ← Analyze Another
              </button>
            </div>
              ...
              {showJob && (
                <div className="border-t border-white/5 px-4 sm:px-5 pb-5 pt-5 space-y-4">
                  ...
                  <div className="flex items-center justify-between">
                    <span className={`text-xs ${jobDesc.length < 50 ? 'text-zinc-600' : 'text-emerald-500'}`}>
                    ...
                    <div className="flex items-start gap-3 p-4 bg-red-500/8 border border-red-500/15 rounded-xl">
                      ...
                      <div className="flex-1">
                        ...
                        <button onClick={handleJobMatch}
                          className="inline-flex items-center gap-1.5 mt-1.5 text-zinc-400 hover:text-white text-sm transition-colors">
                  ...
                  {jobMatch && (
                    <div className="space-y-4">
                      <div className={`text-center p-5 sm:p-6 rounded-xl border ${
                        jobMatch.match_percentage >= 75 ? 'bg-emerald-500/8 border-emerald-500/20'
                        : jobMatch.match_percentage >= 50 ? 'bg-yellow-500/8 border-yellow-500/20'
                        : 'bg-red-500/8 border-red-500/20'
                      }`}>
                        <div className={`text-5xl sm:text-6xl font-black mb-1 ${
                          jobMatch.match_percentage >= 75 ? 'text-emerald-400'
                          : jobMatch.match_percentage >= 50 ? 'text-yellow-400'
                          : 'text-red-400'
                        }`}>{jobMatch.match_percentage}%</div>
                        ...
                        <div className={`mt-2 inline-block px-3 py-1 rounded-full text-xs font-semibold ${
                          jobMatch.match_percentage >= 75 ? 'bg-emerald-500/15 text-emerald-400'
                          : jobMatch.match_percentage >= 50 ? 'bg-yellow-500/15 text-yellow-400'
