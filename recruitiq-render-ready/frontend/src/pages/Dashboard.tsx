      ...
      <div className="max-w-6xl mx-auto relative">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          ...
          <div className="flex items-center gap-3">
            <button onClick={load} disabled={loading}
              className="p-2.5 glass glass-hover rounded-xl text-zinc-400 hover:text-white disabled:opacity-40 transition-colors">
          ...
          <div className={`mb-6 p-4 rounded-xl border flex flex-col sm:flex-row sm:items-center justify-between gap-3 ${
            ...
            <div>
              <p className={`text-sm font-semibold ${
                left === 0 ? 'text-red-300' : left === 1 ? 'text-yellow-300' : 'text-zinc-300'
              }`}>
          ...
          {[
            {
              ...
              sub: left != null ? `${left} slot${left === 1 ? '' : 's'} left` : 'Unlimited',
            },
            {
        ...
        <div className="glass rounded-2xl overflow-hidden">
          <div className="flex items-center justify-between px-4 sm:px-6 py-4 border-b border-white/5">
            ...
            {analyses.length > 6 && (
              <Link to="/history" className="text-indigo-400 hover:text-indigo-300 text-sm font-medium transition-colors">
          ...
          {loading ? (
            ...
            <div className="text-center py-16 px-4">
              ...
              <Link to="/analyzer"
                ...
                <Upload size={15} /> Analyze First Resume
              </Link>
            </div>
          ) : (
              ...
              {analyses.slice(0, 8).map(a => (
                <div key={a.id} className="flex items-center justify-between px-4 sm:px-6 py-4 hover:bg-white/[0.02] transition-colors gap-2">
                  ...
                  <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
                    ...
                    <Link to={`/report/${a.id}`}
                      className="p-2 text-zinc-500 hover:text-indigo-400 rounded-lg hover:bg-white/5 transition-colors">
