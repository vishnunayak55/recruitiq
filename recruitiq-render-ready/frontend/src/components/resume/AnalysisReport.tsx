...
import {
  ...
  Award, Zap, TrendingUp, Copy, Check, Pencil
} from 'lucide-react';
...
const Section = ({ title, icon, children, defaultOpen = true, badge }: any) => {
    ...
    <div className="glass rounded-2xl overflow-hidden">
      <button onClick={() => setOpen(!open)} className="w-full flex items-center justify-between p-5 hover:bg-white/[0.02] transition-colors">
        <div className="flex items-center gap-2.5 text-white font-semibold">
          {icon}{title}
          {badge && <span className="ml-2 px-2 py-0.5 bg-yellow-500/15 text-yellow-400 text-xs font-bold rounded-full border border-yellow-500/25">✦ Premium</span>}
        </div>
...
const CopyButton = ({ text }: { text: string }) => {
  ...
  return (
    <button onClick={handleCopy}
      className="flex items-center gap-1 px-2.5 py-1 bg-white/8 hover:bg-white/12 border border-white/10 rounded-lg text-xs text-zinc-400 hover:text-white transition-colors">
...
Generated by RecruitIQ — recruitiq.ai
  ...
  const handleGenerateRewrite = async () => {
    ...
    try {
      const { data } = await api.post(`/resumes/${analysis.id}/rewrite`);
      setRewriteData(data.rewrite);
      toast.success('Resume rewrite generated!');
    } catch (e: any) {
      toast.error(e?.response?.data?.error || 'Failed to generate rewrite');
  ...
  return (
    <div className="space-y-4 animate-slide-up">
      ...
      {/* ✨ RESUME REWRITE — PREMIUM FEATURE */}
      <Section
        title="Resume Rewrite Suggestions"
        icon={<Pencil size={16} className="text-yellow-400" />}
        defaultOpen={false}
        badge={true}
      >
        {!isPremium ? (
          <div className="text-center py-6">
            ...
            <p className="text-zinc-400 text-sm mb-1">Resume rewrite requires <strong className="text-white">Premium plan</strong></p>
            <p className="text-zinc-500 text-xs mb-4">AI rewrites your weak bullet points with stronger action verbs and metrics</p>
            <button onClick={() => navigate('/pricing')} className="px-5 py-2.5 bg-yellow-500 hover:bg-yellow-400 text-black text-sm font-bold rounded-xl transition-colors">
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-3 p-3 bg-emerald-500/8 border border-emerald-500/20 rounded-xl">
              ...
              <div>
                <p className="text-emerald-400 text-sm font-semibold">Estimated improvement: {rewriteData.score_improvement}</p>
                <p className="text-zinc-500 text-xs">{rewriteData.total_rewrites} bullet points rewritten</p>
              </div>
            </div>

            {/* Summary rewrite */}
            {rewriteData.summary_rewrite?.original && (
              <div className="space-y-2">
                <p className="text-zinc-400 text-xs font-semibold uppercase tracking-wide">Professional Summary</p>
                <div className="p-3 bg-red-500/8 border border-red-500/15 rounded-xl">
                  <div className="flex items-center justify-between mb-1">
                    <p className="text-red-400 text-xs font-semibold">Before</p>
                  </div>
                  <p className="text-zinc-400 text-sm leading-relaxed">{rewriteData.summary_rewrite.original}</p>
                </div>
                <div className="p-3 bg-emerald-500/8 border border-emerald-500/15 rounded-xl">
                  <div className="flex items-center justify-between mb-1">
                    <p className="text-emerald-400 text-xs font-semibold">After</p>
                    <CopyButton text={rewriteData.summary_rewrite.rewritten} />
                  </div>
                  <p className="text-zinc-300 text-sm leading-relaxed">{rewriteData.summary_rewrite.rewritten}</p>
                </div>
              </div>
            )}
            ...
            {/* Bullet rewrites */}
            <div className="space-y-3">
              <p className="text-zinc-400 text-xs font-semibold uppercase tracking-wide">Bullet Point Rewrites</p>
              {rewriteData.rewrites?.map((r: any, i: number) => (
                <div key={i} className="rounded-xl overflow-hidden border border-white/8">
                  <div className="p-3 bg-red-500/5 border-b border-white/5">
                    ...
                    <p className="text-zinc-400 text-sm leading-relaxed">{r.original}</p>
                  </div>
                  <div className="p-3 bg-emerald-500/5">
                    ...
                    <p className="text-zinc-300 text-sm leading-relaxed">{r.rewritten}</p>
                    {r.improvement && (
                      <p className="text-zinc-600 text-xs mt-1.5">✦ {r.improvement}</p>
                    )}
                  </div>
                </div>
              ))}
            ...
            {rewriteData.tips?.length > 0 && (
              <div className="p-4 bg-indigo-500/8 border border-indigo-500/15 rounded-xl">
                <p className="text-indigo-400 text-xs font-semibold mb-2">💡 Additional Tips</p>
                <ul className="space-y-1.5">
                  {rewriteData.tips.map((tip: string, i: number) => (
            ...
            {/* Regenerate button */}
            <button onClick={handleGenerateRewrite} disabled={loadingRewrite}
              className="w-full py-2.5 glass glass-hover text-zinc-400 hover:text-white text-sm rounded-xl flex items-center justify-center gap-2 transition-colors">
              {loadingRewrite && <Loader2 size={14} className="animate-spin" />}
              {loadingRewrite ? 'Regenerating...' : '↻ Regenerate Rewrite'}
            </button>
          </div>
        ) : (
          <div className="text-center py-6 space-y-4">
            <div className="space-y-2">
              <Pencil size={28} className="text-yellow-400 mx-auto" />
              <p className="text-zinc-300 font-medium">AI Resume Rewriter</p>
              <p className="text-zinc-500 text-sm max-w-sm mx-auto">
                Our AI will rewrite your weak bullet points with strong action verbs and metrics — showing before and after for each line.
              </p>
            </div>
            <div className="flex justify-center gap-6 text-xs text-zinc-500">
              <span className="flex items-center gap-1"><Check size={11} className="text-emerald-400" /> Strong action verbs</span>
              <span className="flex items-center gap-1"><Check size={11} className="text-emerald-400" /> Added metrics</span>
              <span className="flex items-center gap-1"><Check size={11} className="text-emerald-400" /> Copy with one click</span>
            </div>
            <button onClick={handleGenerateRewrite} disabled={loadingRewrite}
              className="px-6 py-3 bg-yellow-500 hover:bg-yellow-400 text-black font-bold rounded-xl flex items-center justify-center gap-2 mx-auto transition-colors">
              {loadingRewrite && <Loader2 size={16} className="animate-spin" />}
              {loadingRewrite ? 'Rewriting Resume...' : '✦ Rewrite My Resume'}
            </button>
          </div>
        )}
      </Section>
              ...
              {roadmap.milestones?.map((m: any) => (
                <div key={m.phase} className="p-4 glass rounded-xl">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    {m.level && <span className="px-2 py-0.5 bg-indigo-500/15 text-indigo-400 text-xs rounded-full">{m.level}</span>}
                  ...
                  {m.actions?.length > 0 && <ul className="text-zinc-400 text-xs space-y-0.5 mb-2">{m.actions.slice(0,3).map((a: string, i: number) => <li key={i}>• {a}</li>)}</ul>}
                  {m.resources?.length > 0 && (
                    <div className="flex flex-wrap gap-2 mt-2">
                      {m.resources.map((r: any) => (
                        <a key={r.name} href={r.url} target="_blank" rel="noopener noreferrer"
                          className="px-2 py-0.5 bg-white/5 hover:bg-white/10 border border-white/10 text-zinc-400 hover:text-white text-xs rounded-md transition-colors">
                          {r.name} ↗
                        </a>
                      ))}
                    </div>
